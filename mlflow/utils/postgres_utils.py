...
def where_format(where):
    ...
    if where: 
        _where = []
        for field_op in where:
            field, _ , op = field_op.partition('__')
            oper = _operators.get(op, op) or '='
            clause = '%s %s %%(%s)s' % (field, oper, field_op)
            _where.append(clause)
        return ' WHERE ' + ' AND '.join(_where)
    else:
        return ''

def update_format(values):
    _update = []
    for k, v in values.items():
        f, _, op = k.partition('__')
        clause = _update_operators[op] % {'key': k, 'val': v, 'field': f}
        _update.append(clause)
    return ','.join(_update)
...
class PostgresDB(object):
    ...
    def query_one(self, qry, params=None):
        with closing(self.get_connection()) as conn:
            with closing(conn.cursor()) as cursor:
                cursor.execute(qry, params)
    ...
    def table_exists(self, table_name):
        sql = 'SELECT tablename FROM pg_tables WHERE schemaname=%s and tablename=%s'
        return self.query_one(sql, ('public', table_name)) is not None
    ...
    def create_table(self, table_name, schema):
        """
            >>> db.create_table('mlflow_experiments','''id SERIAL PRIMARY KEY, name TEXT''')
            >>> db.table_exists('mlflow_experiments')
            True
            >>> db.drop_table('mlflow_experiments');
            >>> db.table_exists('mlflow_experiments')
            False
        """
        if not self.table_exists(table_name):
            self.execute('CREATE TABLE %s (%s)' % (table_name, schema))

    def insert(self, table_name, values, returning=None):
        """
            >>> db.insert('mlflow_experiments', {'name':'xxx'})
            1
            >>> db.insert('mlflow_experiments', values={'name':'yyy'}, returning='name')
            ('yyy',)
        """
        _values = ','.join(['%%(%s)s' % v for v in values.keys()])
        keys = ','.join(values.keys())
        sql = 'INSERT INTO %s (%s) VALUES (%s)' % (table_name, keys, _values)
        if returning:
    ...
    def update(self, table_name, values, where=None, returning=None):
        """
        ...
        sql = 'UPDATE %s SET %s' % (table_name, update_format(values))
        sql = cursor.mogrify(sql, values)
        if where:
            sql += cursor.mogrify(where_format(where), where)
        if returning:
            sql += b" RETURNING %s" % returning.encode()
...
class Connection(object):
    ...
    def __init__(self, uri ,hstore=False,log=None,logf=None,min=1,max=5,
